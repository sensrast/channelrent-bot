...
async def browse_panel(update: Update, context: ContextTypes.DEFAULT_TYPE, page=0):
    ...
    kbrows = []
    for c in rows:
        kbrows.append([(f"📋 {c['title'][:25]}", f"adv:ch:{c['channel_id']}")])
...
async def filters_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    f = _filters(context)
...
async def sort_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    rows = [
...
async def category_picker(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query
    await q.answer()
    cats = await list_all_categories()
...
async def apply_filter(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if kind == "reset":
        ...
        f["sort"] = val
    await filters_panel(update, context) if kind in ("c","reset") else await browse_panel(update, context)
