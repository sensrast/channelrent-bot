...
async def _force_sub_check(bot, user_id):
    ...
    raw_ch = (await db.fetchval("SELECT value FROM platform_settings WHERE key='force_sub_channel'") or "").strip()
    ch = normalize_force_sub_channel(raw_ch)
    if not en or not ch:
...
def _force_sub_keyboard(channel):
    handle = (channel or "").lstrip("@").strip()
    if handle.lstrip("-").isdigit() or not handle:
        url = None
    else:
    ...
    if not bot_ok:
        ...
        try:
            await q.edit_message_text(
                "🎉 Verified! Referral reward credited. Tap /start to begin.\n\n"
                "(Note: membership check was skipped because the bot is not an admin "
                "of the force-sub channel. Admin has been notified.)"
            )
        except Exception: pass
        return
    if not is_mem:
