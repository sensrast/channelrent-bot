...
AP_PICK_LIST, AP_PICK_CHANNELS, AP_GET_CONTENT, AP_ASK_BUTTONS, AP_GET_BUTTONS, AP_CONFIRM = range(6)
...
async def _show_confirm(target, context):
    ...
    txt = (f"📋 <b>Confirm Broadcast</b>\n\n"
           ...
           f"Posts will NOT be auto-deleted or pinned.")
    ...
    for cid in selected_ids:
        ...
        try:
            ...
            if s["content_type"] == "text":
                await context.bot.send_message(chat_id, s["content_text"], reply_markup=markup,
                                               disable_web_page_preview=False)
            elif s["content_type"] == "photo":
                await context.bot.send_photo(chat_id, s["media_file_id"],
                                             caption=s["caption"] or None, reply_markup=markup)
            elif s["content_type"] == "document":
                await context.bot.send_document(chat_id, s["media_file_id"],
                                                caption=s["caption"] or None, reply_markup=markup)
            sent += 1
        except Exception as e:
