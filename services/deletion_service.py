...
async def process_expired(bot: Bot):
    ...
    for b in rows:
        try:
            ...
            if not ch:
                await settle_booking(b["booking_id"], "message_lost"); continue
    ...
    for b in rows:
        try:
            ...
            await notify(bot, b["advertiser_id"], "owner_deleted_post",
                f"⚠️ <b>Your ad was removed early by the channel owner</b>\nBooking: <code>{b['booking_ref']}</code>\nChannel: {ch['title']}\n💰 Full refund: {refund} credits returned to your wallet.",
                booking_id=b["booking_id"])
            await notify(bot, b["owner_id"], "owner_deleted_post",
                f"ℹ️ Booking <code>{b['booking_ref']}</code> on {ch['title']} was no longer present. The advertiser was fully refunded.",
                booking_id=b["booking_id"])
        except Exception as e:
